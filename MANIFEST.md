...
| [2b] | `02b_abstract_top_leaf_stats.py` | top-leaf CCI two-way ANOVA at 30 and 60 DAT -> `Table_abstract_top_leaf_stats.csv` |
...
A consolidated `Tables.docx` is also produced by `06_tables.py`, with each table's caption read
from `manuscript/sources/09_Table_legends.md` (vendored here for exactly this purpose — it is a
minimal excerpt of the four main-table legends, not the full manuscript source tree, which stays
...
| `anova_F_authors_table1.csv` | not regenerated by this pipeline | published ANOVA F-values for this experiment [29], used as a cross-check on `02_descriptives.py`'s own output |
